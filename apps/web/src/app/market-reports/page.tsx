import type { Metadata } from "next";
import Link from "next/link";

import { SavedSearchPrompt } from "@/components/leads/SavedSearchPrompt";
import { localitiesWithContent } from "@/config/localities";
import { site } from "@/config/site";
import { formatPrice, formatPriceCompact } from "@/lib/format";
import { getListingProvider } from "@/lib/listings";

/**
 * Area market reports.
 *
 * Two jobs. For search engines, it's a recurring-freshness page targeting "{sector} property
 * prices" and "tricity market report". For the business, it's the hook for a monthly email — the
 * cheapest way to stay in front of past clients and future sellers without ever writing "just
 * checking in".
 *
 * Stats are derived from live inventory, so the page updates itself as the market moves.
 * Revalidated hourly.
 */

export const revalidate = 3600;

export const metadata: Metadata = {
  title: `${site.market.name} Market Reports`,
  description:
    `Current property prices, inventory and days on market for sectors and phases across ` +
    `Chandigarh, Mohali and Kharar.`,
};

export default async function MarketReportsPage() {
  const provider = getListingProvider();

  const reports = await Promise.all(
    localitiesWithContent().map(async (locality) => ({
      locality,
      stats: await provider.getMarketStats({
        citySlug: locality.citySlug,
        localitySlug: locality.slug,
      }),
    })),
  );

  const withStats = reports.filter((r) => r.stats !== null);

  const totalActive = withStats.reduce((sum, r) => sum + (r.stats?.activeCount ?? 0), 0);

  /*
   * A median of medians, not a true median — computing the real figure needs the full listing
   * set rather than per-locality aggregates. It is close enough for a headline number and is
   * labelled as indicative, but do not quote it as a market median.
   */
  const overallMedian = withStats.length
    ? Math.round(
        withStats.reduce((sum, r) => sum + (r.stats?.medianListPrice ?? 0), 0) /
          withStats.length,
      )
    : 0;

  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <header className="max-w-3xl">
        <h1 className="font-display text-4xl font-semibold text-sand-950 sm:text-5xl">
          {site.market.name} market reports
        </h1>
        <p className="mt-4 text-lg leading-relaxed text-sand-700">
          Where prices actually are right now, sector by sector. Updated continuously from current
          inventory — not last quarter&rsquo;s summary.
        </p>
      </header>

      <section className="mt-10 rounded-card bg-brand-800 p-8">
        <h2 className="font-display text-2xl font-semibold text-white">
          The tricity at a glance
        </h2>
        <dl className="mt-6 grid grid-cols-2 gap-6 sm:grid-cols-3">
          <div>
            <dt className="text-sm text-brand-200">Properties for sale</dt>
            <dd className="mt-1 font-display text-3xl font-semibold text-white">
              {totalActive}
            </dd>
          </div>
          <div>
            <dt className="text-sm text-brand-200">Indicative median</dt>
            <dd className="mt-1 font-display text-3xl font-semibold text-white">
              {overallMedian ? formatPriceCompact(overallMedian) : "—"}
            </dd>
          </div>
          <div>
            <dt className="text-sm text-brand-200">Areas tracked</dt>
            <dd className="mt-1 font-display text-3xl font-semibold text-white">
              {withStats.length}
            </dd>
          </div>
        </dl>
      </section>

      <section className="mt-12">
        <h2 className="font-display text-2xl font-semibold text-sand-950">By area</h2>

        {/* Horizontal scroll rather than a cramped stack — comparison is the point of this table. */}
        <div className="mt-6 overflow-x-auto">
          <table className="w-full min-w-[46rem] border-collapse text-sm">
            <thead>
              <tr className="border-b border-sand-300 text-left">
                <th scope="col" className="pb-3 pr-4 font-semibold text-sand-900">Area</th>
                <th scope="col" className="pb-3 pr-4 font-semibold text-sand-900">City</th>
                <th scope="col" className="pb-3 pr-4 font-semibold text-sand-900">For sale</th>
                <th scope="col" className="pb-3 pr-4 font-semibold text-sand-900">Median price</th>
                <th scope="col" className="pb-3 pr-4 font-semibold text-sand-900">₹ / sq ft</th>
                <th scope="col" className="pb-3 font-semibold text-sand-900">Days listed</th>
              </tr>
            </thead>
            <tbody>
              {withStats.map(({ locality, stats }) => (
                <tr key={`${locality.citySlug}/${locality.slug}`} className="border-b border-sand-100">
                  <th scope="row" className="py-3 pr-4 text-left font-medium">
                    <Link
                      href={`/localities/${locality.citySlug}/${locality.slug}`}
                      className="text-brand-700 hover:underline"
                    >
                      {locality.name}
                    </Link>
                  </th>
                  <td className="py-3 pr-4 text-sand-700">{locality.cityName}</td>
                  <td className="py-3 pr-4 tabular-nums text-sand-800">{stats!.activeCount}</td>
                  <td className="py-3 pr-4 tabular-nums text-sand-800">
                    {formatPrice(stats!.medianListPrice)}
                  </td>
                  <td className="py-3 pr-4 tabular-nums text-sand-800">
                    {/* Zero means no listing here had a usable area — a dash is honest, ₹0 is not. */}
                    {stats!.medianPricePerSqft > 0
                      ? `₹${stats!.medianPricePerSqft.toLocaleString("en-IN")}`
                      : "—"}
                  </td>
                  <td className="py-3 tabular-nums text-sand-800">
                    {stats!.medianDaysOnMarket}
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>

        <p className="mt-4 text-xs leading-relaxed text-sand-500">
          Figures reflect currently listed inventory and are a snapshot, not a valuation. Small
          areas can swing sharply month to month on a handful of sales — treat single-month
          movement with caution. Per-sq-ft rates mix plots and built property where both are
          listed, so compare like with like before drawing conclusions.
        </p>
      </section>

      <div className="mt-14">
        <SavedSearchPrompt
          searchDescription="tricity market updates"
          queryString=""
        />
      </div>

      <section className="mt-12 rounded-card border border-sand-200 bg-sand-100 p-8 text-center">
        <h2 className="font-display text-2xl font-semibold text-sand-950">
          Want to know what your property is worth in this market?
        </h2>
        <p className="mx-auto mt-2 max-w-xl leading-relaxed text-sand-700">
          Area medians are a starting point. Your property is not a median.
        </p>
        <Link
          href="/home-value"
          className="mt-6 inline-block rounded-md bg-brand-700 px-6 py-3 text-sm font-semibold text-white hover:bg-brand-800"
        >
          Get a real valuation
        </Link>
      </section>
    </div>
  );
}
