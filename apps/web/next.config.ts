...
const nextConfig: NextConfig = {
  ...
  /*
   * Leftovers from the US build, which called these pages "neighborhoods". The route is
   * /localities now.
   *
   * Permanent (308) rather than temporary: the old path is never coming back, and a 301/308 is
   * what tells a crawler to move any accumulated authority across instead of indexing both. Both
   * spellings are covered because half the world writes the U and a visitor guessing a URL should
   * land somewhere useful rather than on a 404.
   */
  async redirects() {
    return [
      { source: "/neighborhoods", destination: "/localities", permanent: true },
      { source: "/neighbourhoods", destination: "/localities", permanent: true },
      { source: "/neighborhoods/:path*", destination: "/localities/:path*", permanent: true },
      { source: "/neighbourhoods/:path*", destination: "/localities/:path*", permanent: true },
    ];
  },
