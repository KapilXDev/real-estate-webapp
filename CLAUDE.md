...
- `apps/web/AGENTS.md` + `CLAUDE.md` are **auto-generated by `next dev`**; committing them keeps the
  tree clean.
- **`apps/*/next-env.d.ts` is gitignored, and must stay that way.** Next rewrites it depending on
  the last command — `next dev` points it at `.next/dev/types/`, `next build` at `.next/types/` —
  so committing it produces a spurious diff on every branch AND references a path under the
  gitignored `.next/`, which does not exist on a fresh checkout.
- **⚠️ Do NOT add the Next apps to a bare `tsc --noEmit` step.** Their tsconfigs include
  `.next/types/**`, so a clean checkout fails with TS2307 before reaching any of our code — this
  is why CI had never been green. `next build` generates those types and typechecks the app, and
  that is the supported way to check a Next app.
